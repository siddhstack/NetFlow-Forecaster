...
from evaluate_model import parse_args as eval_parse_args
import subprocess
import sys
    ...
    for strategy in strategies:
        ...
        for attempt_idx in range(args.attempts_per_strategy):
            ...
            try:
                ...
                print(f"Error training {strategy}/{candidate.id}: {exc}")
                summary = {"overall": {"normalized_quality_pct": 0.0}}
            ...
            csv_rows.append({
                ...
                "quality_pct": quality,
                "traffic_spike_f1": float(summary.get("by_feature", {}).get("traffic_mbps", {}).get("spike_f1", 0.0)),
                "latency_spike_f1": float(summary.get("by_feature", {}).get("latency_ms", {}).get("spike_f1", 0.0)),
                "packet_loss_spike_f1": float(summary.get("by_feature", {}).get("packet_loss_pct", {}).get("spike_f1", 0.0)),
